...
ggplot(data = , 
       aes(x = , y = )) +
  geom_point(alpha=)+
  geom_smooth(method=, aes(color=), size=)
...
ggplot(data = , 
       aes(x = , y = )) +
  geom_point(size=, aes(shape=, color=)) 
...
left <- plot_grid(, , nrow=, align=)

#ggsave(a, file="filenamehere.extension")
ggsave(, file="~/../Desktop/left.jpeg", 
